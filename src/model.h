...
public:
	...
	string docmapfile;
	string citationmapfile; //file that contains citation map [string -> int id]
    ...
    mapid2word id2word; // word map [int => string]
	 ...
	 //add by liu liu
	mapid2citation id2citation; //citation map [in => string]
	...
	//add by liuliu
	int save_model_likelihood(string filename);
	...
	//add by liu liu
	int init_evaluate();
	int load_model_evaluate(string model_name);
