...
void model::set_default_values() {
	...
	citationmapfile = "citationmap.txt"; // add by liu liu
...
int model::init(int argc, char ** argv) {
	...
	} else if(model_status ==MODEL_STATUS_EVALUATE){
		if(init_evaluate()){
			return 1;
		}
	}
...
double model::compute_likelihood(){
	...
	double lngammaalpha = lngamma(alpha, sign);
	double Klngammaalpha = lngamma(K*alpha, sign);
	
	for(int m = 0;m<M;m++){
		double tmp = 0.0;
		for(int k = 0;k<K;k++){
			tmp +=lngamma(nd[m][k]+alpha, sign)-lngammaalpha;			
		}
		doctopic +=tmp + Klngammaalpha - lngamma(ndsum[m]+K*alpha, sign);
	}
...
void model::evaluate(){
	...
	map<string, int>::iterator name2idit;
	...
	double p5tmp, p10tmp, rpretmp, meanaptmp, recall5tmp, recall10tmp, bpref10tmp;
		...
		fprintf(fout, "²ßÂÔÒ»: no inference.\n");
